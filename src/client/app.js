...
import { Card, Button, CardTitle, CardText, Row, Col, Form, FormControl, FormGroup, ControlLabel, Label, Input } from 'reactstrap';
...
class App extends React.Component {
    ...
    render() {
        ...
        return (
            <div>
                <Map className="leaflet-map" center={position} zoom={this.state.zoom} >
                    <TileLayer
                        attribution='&amp;copy <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
                        url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                    />
                    {this.state.haveUsersLocation ? <Marker position={position}
                        icon={myIcon}>
                        <Popup>
                    ...
                    {listATM.map((value, index) => {
                        return <Marker key={index} position={[value.latitude, value.longitude]} icon={myIcon}></Marker>
                    })}
                </Map>
